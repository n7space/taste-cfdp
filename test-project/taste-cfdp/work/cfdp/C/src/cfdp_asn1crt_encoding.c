...
{
	cfdp_BitStream_AttachBuffer(pBitStrm, buf, count);
...
void cfdp_BitStream_AppendBits(BitStream* pBitStrm, const byte* srcBuffer, int nbits)
{
	byte lastByte = 0;
    ...
    cfdp_BitStream_EncodeOctetString_no_length(pBitStrm, srcBuffer, bytesToEncode);

    if (remainingBits > 0) {
		lastByte = (byte)(srcBuffer[bytesToEncode] >> (8 - remainingBits));
		cfdp_BitStream_AppendPartialByte(pBitStrm, lastByte, (byte)remainingBits, FALSE);
	}
}
...
/*
flag cfdp_BitStream_ReadByte2(BitStream2* pBitStrm, byte* v)
{
	int cb = pBitStrm->currentBit;
...
static void cfdp_BitStream_EncodeNonNegativeInteger32Neg(BitStream* pBitStrm,
	asn1SccUint32 v,
	flag negate)
{
	int cc;
	...
	if (pbits) {
		cc -= pbits;
		cfdp_BitStream_AppendPartialByte(pBitStrm, (byte)(v >> cc), (byte)pbits, negate);
	}
...
static flag cfdp_BitStream_DecodeNonNegativeInteger32Neg(BitStream* pBitStrm,
	asn1SccUint32* v,
	int nBits)
{
	byte b;
	*v = 0;
	while (nBits >= 8) {
		*v <<= 8;
		if (!cfdp_BitStream_ReadByte(pBitStrm, &b))
			return FALSE;
		*v |= b;
		nBits -= 8;
	}
	...
	{
		*v <<= nBits;
		if (!cfdp_BitStream_ReadPartialByte(pBitStrm, &b, (byte)nBits))
			return FALSE;
		*v |= b;
	}
...
#if WORD_SIZE==8
	if (v<0x100000000LL)
		cfdp_BitStream_EncodeNonNegativeInteger32Neg(pBitStrm, (asn1SccUint32)v, 0);
	else {
...
#else
	cfdp_BitStream_EncodeNonNegativeInteger32Neg(pBitStrm, v, 0);
#endif
}
...
flag cfdp_BitStream_DecodeNonNegativeInteger(BitStream* pBitStrm, asn1SccUint* v, int nBits)
{
	...
	if (nBits <= 32)
	{
		ret = cfdp_BitStream_DecodeNonNegativeInteger32Neg(pBitStrm, &lo, nBits);
		*v = lo;
		return ret;
	}

	ret = cfdp_BitStream_DecodeNonNegativeInteger32Neg(pBitStrm, &hi, 32) && cfdp_BitStream_DecodeNonNegativeInteger32Neg(pBitStrm, &lo, nBits - 32);
	...
	*v |= lo;
	return ret;
#else
	return cfdp_BitStream_DecodeNonNegativeInteger32Neg(pBitStrm, v, nBits);
#endif
}
...
flag cfdp_BitStream_EncodeOctetString_no_length (BitStream* pBitStrm, const byte* arr, int nCount) {
	...
	if (cb == 0) {
		...
		ret = cfdp_BitStream_AppendByteArray(pBitStrm, arr, (size_t) nCount);
/*
		for (i1 = 0; (i1 < (int)nCount) && ret; i1++)
		{
			ret = cfdp_BitStream_AppendByte0(pBitStrm, arr[i1]);
		}
*/		
	}
...
flag cfdp_BitStream_DecodeOctetString_no_length(BitStream* pBitStrm, byte* arr, int nCount) {
    ...
    if (cb == 0) {
		...
		ret = cfdp_BitStream_ReadByteArray(pBitStrm, arr, nCount);
        //for (i1 = 0; (i1 < nCount) && ret; i1++)
        //{
        //    ret = cfdp_BitStream_ReadByte(pBitStrm, &arr[i1]);
        //}
    }
...
flag cfdp_BitStream_EncodeOctetString_fragmentation(BitStream* pBitStrm, const byte* arr, int nCount) {
	...
	while (nRemainingItemsVar1 >= 0x4000 && ret)
	{
		if (nRemainingItemsVar1 >= 0x10000)
		{
			...
			cfdp_BitStream_EncodeConstraintWholeNumber(pBitStrm, 0xC4, 0, 0xFF);
		}
		else if (nRemainingItemsVar1 >= 0xC000)
		{
			nCurBlockSize1 = 0xC000;
			cfdp_BitStream_EncodeConstraintWholeNumber(pBitStrm, 0xC3, 0, 0xFF);
		}
		else if (nRemainingItemsVar1 >= 0x8000)
		{
			nCurBlockSize1 = 0x8000;
			cfdp_BitStream_EncodeConstraintWholeNumber(pBitStrm, 0xC2, 0, 0xFF);
		}
		else
		{
			nCurBlockSize1 = 0x4000;
			cfdp_BitStream_EncodeConstraintWholeNumber(pBitStrm, 0xC1, 0, 0xFF);
		}
	...
	if (ret) {
		if (nRemainingItemsVar1 <= 0x7F)
			cfdp_BitStream_EncodeConstraintWholeNumber(pBitStrm, nRemainingItemsVar1, 0, 0xFF);
		else
		{
			cfdp_BitStream_AppendBit(pBitStrm, 1);
			cfdp_BitStream_EncodeConstraintWholeNumber(pBitStrm, nRemainingItemsVar1, 0, 0x7FFF);
		}
...
flag cfdp_BitStream_DecodeOctetString_fragmentation(BitStream* pBitStrm, byte* arr, int* nCount, asn1SccSint asn1SizeMax) {
	...
	ret = cfdp_BitStream_DecodeConstraintWholeNumber(pBitStrm, &nRemainingItemsVar1, 0, 0xFF);
	if (ret) {
		...
		if (ret) {
			if ((nRemainingItemsVar1 & 0x80)>0)
			{
				...
				ret = cfdp_BitStream_DecodeConstraintWholeNumber(pBitStrm, &len2, 0, 0xFF);
				if (ret) {
...
flag cfdp_BitStream_EncodeOctetString(BitStream* pBitStrm, const byte* arr, int nCount, asn1SccSint asn1SizeMin, asn1SccSint asn1SizeMax) {
	flag ret= (nCount >= asn1SizeMin && nCount <= asn1SizeMax);

	if (ret) {
		if (asn1SizeMax < 65536) {
			if (asn1SizeMin != asn1SizeMax) {
				cfdp_BitStream_EncodeConstraintWholeNumber(pBitStrm, nCount, asn1SizeMin, asn1SizeMax);
			}
			ret = cfdp_BitStream_EncodeOctetString_no_length(pBitStrm, arr, nCount);
		}
		else {
			ret = cfdp_BitStream_EncodeOctetString_fragmentation(pBitStrm, arr, nCount);
		}
	}
...
flag cfdp_BitStream_DecodeOctetString(BitStream* pBitStrm, byte* arr, int* nCount, asn1SccSint asn1SizeMin, asn1SccSint asn1SizeMax) {
	flag ret = TRUE;
	if (asn1SizeMax < 65536) {
		asn1SccSint nCountL;
		if (asn1SizeMin < asn1SizeMax) {
			ret = cfdp_BitStream_DecodeConstraintWholeNumber(pBitStrm, &nCountL, asn1SizeMin, asn1SizeMax);
		}
		else {
			ret = TRUE;
			nCountL = asn1SizeMin;
		...
		*nCount = (int)nCountL;
		...
		if (ret) {
			cfdp_BitStream_DecodeOctetString_no_length(pBitStrm, arr, *nCount);
		}
	}
	else {
		ret = cfdp_BitStream_DecodeOctetString_fragmentation(pBitStrm, arr, nCount, asn1SizeMax);
	}
	return ret;
}
