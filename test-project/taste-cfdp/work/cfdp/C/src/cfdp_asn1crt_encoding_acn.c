...
void cfdp_Acn_Enc_Int_PositiveInteger_ConstSize(BitStream* pBitStrm, asn1SccUint intVal, int encodedSizeInBits)
{
	int nBits = 0;
	...
	/*Encode number */
	cfdp_BitStream_EncodeNonNegativeInteger(pBitStrm, intVal);

	CHECK_BIT_STREAM(pBitStrm);
}
void cfdp_Acn_Enc_Int_PositiveInteger_ConstSize_8(BitStream* pBitStrm, asn1SccUint intVal)
{
	cfdp_BitStream_AppendByte0(pBitStrm, (byte)intVal);
	CHECK_BIT_STREAM(pBitStrm);
}

static void cfdp_Acn_Enc_Int_PositiveInteger_ConstSize_big_endian_B(BitStream* pBitStrm,
	asn1SccUint intVal,
	int size)
{
	int i = 0;
	...
	for (i = 0; i<size; i++) {
		...
		cfdp_BitStream_AppendByte0(pBitStrm, ByteToEncode);
		mask >>= 8;
	}
	CHECK_BIT_STREAM(pBitStrm);
}

void cfdp_Acn_Enc_Int_PositiveInteger_ConstSize_big_endian_16(BitStream* pBitStrm, asn1SccUint intVal)
{
	cfdp_Acn_Enc_Int_PositiveInteger_ConstSize_big_endian_B(pBitStrm, intVal, 2);
}

void cfdp_Acn_Enc_Int_PositiveInteger_ConstSize_big_endian_32(BitStream* pBitStrm, asn1SccUint intVal)
{
	cfdp_Acn_Enc_Int_PositiveInteger_ConstSize_big_endian_B(pBitStrm, intVal, 4);
}
...
flag cfdp_Acn_Dec_Int_PositiveInteger_ConstSize(BitStream* pBitStrm, asn1SccUint* pIntVal, int encodedSizeInBits)
{
	asn1SccUint tmp = 0;
	if (cfdp_BitStream_DecodeNonNegativeInteger(pBitStrm, &tmp, encodedSizeInBits))
	{
		*pIntVal = tmp;
		return TRUE;
	}
...
flag cfdp_Acn_Dec_Int_PositiveInteger_ConstSize_8(BitStream* pBitStrm, asn1SccUint* pIntVal)
{
	byte v = 0;
	if (!cfdp_BitStream_ReadByte(pBitStrm, &v))
		return FALSE;
	*pIntVal = v;
	return TRUE;
}

static flag cfdp_Acn_Dec_Int_PositiveInteger_ConstSize_big_endian_N(BitStream* pBitStrm,
	asn1SccUint* pIntVal,
	int SizeInBytes)
{
	int i;
...
flag cfdp_Acn_Dec_Int_PositiveInteger_ConstSize_big_endian_16(BitStream* pBitStrm, asn1SccUint* pIntVal)
{
	return cfdp_Acn_Dec_Int_PositiveInteger_ConstSize_big_endian_N(pBitStrm, pIntVal, 2);
}

flag cfdp_Acn_Dec_Int_PositiveInteger_ConstSize_big_endian_32(BitStream* pBitStrm, asn1SccUint* pIntVal)
{
	return cfdp_Acn_Dec_Int_PositiveInteger_ConstSize_big_endian_N(pBitStrm, pIntVal, 4);
}
...
flag cfdp_BitStream_ReadBitPattern(BitStream* pBitStrm, const byte* bitPattern, int nBitsToRead, flag* pBoolValue)
{
	...
	*pBoolValue = TRUE;
	for (i = 0; i<nBytesToRead; i++) {
		if (!cfdp_BitStream_ReadByte(pBitStrm, &curByte))
			return FALSE;
        *pBoolValue = *pBoolValue && (curByte == bitPattern[i]);
	}

	if (nRemainingBitsToRead > 0) {
		if (!cfdp_BitStream_ReadPartialByte(pBitStrm, &curByte, (byte)nRemainingBitsToRead))
			return FALSE;
...
#define Acn_enc_real_big_endian(type)       \
    ...
    if (!RequiresReverse()) {   \
        for(i=0;i<(int)sizeof(dat1);i++)        \
            cfdp_BitStream_AppendByte0(pBitStrm,dat1.b[i]);  \
    } else {    \
...
#define Acn_dec_real_big_endian(type)   \
    ...
    if (!RequiresReverse()) {       \
        for(i=0;i<(int)sizeof(dat1);i++) {  \
            if (!cfdp_BitStream_ReadByte(pBitStrm, &dat1.b[i]))  \
                return FALSE;       \
        }                           \
    } else {                        \
        for(i=(int)(sizeof(dat1)-1);i>=0;i--) {         \
